...
#
#   C:/User/frr/AppData/Local/Programs/Python/Python38/MisProgramas/
3   PLANO INCLINADO/Proyecto IoT/
#   plano_inclinado_03.py
#
#  Requiere tener corriendo el programa de Pyhton:
#
#*******************************************************/
...
void loop() {
  ...
  if(!digitalRead(4)){
    angulo_objetivo = 50; //40; // Angulo de 40 grados. [30 grados EFECTIVOS]. 20 grados
    inicio_secuencia = 1;
  }

  if(!digitalRead(5)){
    angulo_objetivo = 65; // 50// Angulo de 60 grados [50 grados EFECTIVOS]. 30 grados
    inicio_secuencia = 1;
  }
...
void recibe_msgs_python(){
   ...
   if (Serial.available() > 0) {
    delay(20); // Delay para dar tiempo al buffer para que reciba todos los caracteres
    cmd_recibido_python = "";  // String para almacenar todos los caracteres recibidos
    // ---------------------------------
