...
class Marcadores:
    ...
    def dibujar_linea_horizontal(self,frame):
        ...
        cv2.putText(frame, "{:.1f} cm".format(38.0), (int(mX), int(mY - 10)),
        ...
        #     la camara WEB se encuentra totalmente HORIZONTAL
        # -------------------------------------------------------------
        # --- Comprueba que el plano inicializa con angulo = 0 grados ---
##        dy = yB - yA
##        if np.absolute(dy) > 10:
##            # --- Calibrar el Plano Inclinado en angulo CERO GRADOS ---
##            cv2.putText(frame, "*** ! CALIBRAR *** dy = "+str(dy), (int(mX), int(mY - 10)),
##                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2)
##            self.calibrar = int(dy)
##            print("FUERA DE RANGO :: dy>3 :: dy = ",int(dy))
##        else:
##            # --- Longitud de la Hipotenusa (Plano Inclinado) ---
##            cv2.putText(frame, "{:.1f} cm".format(38.0), (int(mX), int(mY - 10)),
##                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)
##            self.calibrar = int(dy)
##            print("DENTRO de rango :: dy<3 :: dy = ",int(dy))
...
##        return calibrar   
    ...
    def dibujar_triangulo_rectangulo(self,frame):
        ...
        if D != 0:
            a = round(a*38.0/D,1) # <- cm
        
        # ----------------------------------------------------
        #     h = Altua del triangulo rectangulo
        # ----------------------------------------------------
        dx = xA - xA
        dy = yB - yA
        ...
        if D == 0:
            ...
            angulo = round(angulo,2)  # int(angulo*10)/10

        # --- Punto medio de la base del triangulo ---
        (aqX, aqY) = self.midpoint((xA, yB), (xB, yB))

        # --- Angulo del Plano Inclinado ---
        cv2.putText(frame, str(angulo)+" grados", (int(coordX_text_angulo), int(coordY_text_angulo)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)
